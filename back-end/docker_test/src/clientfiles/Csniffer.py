...
import time
import sys
from CDBController  import *

def getPacketInfo(networkInterface=None,host=None,port=None):
    if networkInterface is None:
        raise Exception("Please specify an interface.")
    else:
        print("listening on %s" % networkInterface)
        capture = pyshark.LiveCapture(interface=networkInterface)
        dbc = clientserverDbcontroller(host,port)
        for packet in capture.sniff_continuously():
            try:
                ...
                dbc.insertPacketInfo(pktInfo, commande = "ip_layer")
