...
class Agent:
    def __init__(self, initdic):
        ...
        self.addr = initdic.get("client_ip")
...
def search(value):
    ...
    return content
def insertPacket(value):
    command = value.get("commande")
    packt = value.get("packetDict")
    insertPacketInfo(packt, command)
    answer ="packet  added"
    content = {"result": answer}
    return content
...
class Message:
    ...
    def _take_action(self,action,value):
        ...
        "add":add,
        "print":print_agent,
	"insertPacketInfo":insertPacket,
        }
        value['client_ip'] = self.addr[0]
        value['client_port'] = self.addr[1]
